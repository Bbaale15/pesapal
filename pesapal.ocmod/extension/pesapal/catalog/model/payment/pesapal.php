...
class Pesapal extends \Opencart\System\Engine\Model {
	...
	public function getIpnId(string $token): string {
		...
		$ipn_url  = HTTPS_SERVER . 'index.php?route=extension/pesapal/payment/ipn';
	...
	public function submitOrder(array $order_info, string $token, string $ipn_id): array {
		...
		$callback_url       = HTTPS_SERVER . 'index.php?route=extension/pesapal/payment/callback';
		...
		$payload = [
			...
			'billing_address' => [
				...
				'country_code'  => $order_info['payment_iso_code_2'] ?? 'UG',
	...
	public function mapStatusToOrderStatusId(string $pesapal_status): int {
		return match (strtoupper($pesapal_status)) {
			'COMPLETED' => (int)$this->config->get('payment_pesapal_processing_status_id'),
			'FAILED',
			'INVALID'   => (int)$this->config->get('payment_pesapal_failed_status_id'),
			'CANCELLED' => (int)$this->config->get('payment_pesapal_cancelled_status_id'),
			default     => (int)$this->config->get('payment_pesapal_pending_status_id'),
		};
	}
